...
async function postToDiscord(manga, chapterTitle, chapterId, chapterDate) {
  ...
  try {
    ...
    const chapterUrl = `https://zoro-to.kesug.com/readList/${manga.mangaId}/${chapterId}`;
    ...
    for (const manga of mangaList) {
      ...
      if (firstChapter) { // Check if there is at least one chapter
        ...
        if (!postedChapters.has(chapterId)) {
          postToDiscord(manga, chapterTitle, chapterId, chapterDate);
          ...
          if (postedChapters.size > 100) {
            ...
            savePostedChapters(); // Save after deleting an item
          }
        }
      }
    }
...
loadPostedChapters().then(() => {
  ...
  setInterval(checkForUpdates, 60000); // For example, check for updates every minute
