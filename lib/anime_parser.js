...
export const scrapeTrendingManga = async () => {
  try {
    ...
    $('.ts-wpop-tab.active').each((i, el) => {
      $(el).closest('.ts-wpop-series-gen').next().find('.series').each((i, el) => {
        const mangaId = new URL($(el).attr('href')).pathname.split('/').slice(-2)[0]; // Extract manga ID from URL
        const mangaTitle = $(el).text().trim();
        const mangaImg = $(el).find('img').attr('src').replace(/.*?\/fit-in\/\d+x\d+\/filters:format\(webp\)\/(.*)/, '$1'); // Remove unwanted portion of image URL
        const mangaUrl = new URL($(el).attr('href'), trending_url).pathname.replace('/manga/', ''); // Remove unwanted portion of manga URL
        const latestChapter = $(el).closest('li').find('.epxs').text().trim().replace('Chapter ', ''); // Extract chapter number
        const rating = parseFloat($(el).closest('.rt').find('.numscore').text().trim()); // Parse rating as float
