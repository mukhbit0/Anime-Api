...
async function scrapeToonItems(pageNumber) {
    const url = `https://www.toongod.org/page/${pageNumber}/?s&post_type=wp-manga&adult=0&m_orderby=latest`;
    let m_list = [];

    try {
        // Scrape manga items on the current page
        const mangaPageData = await scrapeToonCurrentPage(url);
...
async function scrapeToonCurrentPage(url) {
    let m_list = [];
    ...
    try {
        ...
        $('.c-tabs-item').each((index, element) => {
            const $item = $(element);

            const image = $item.find('.tab-thumb img').attr('data-src');
            // Remove the dimensions from the image URL
            const imageUrl = image.replace(/-\d+x\d+/, '');

            const mangaUrl = $item.find('.post-title a').attr('href');
            const title = $item.find('.post-title a').text().trim();

            const alternative = $item.find('.mg_alternative .summary-content').text().trim();
            const authors = $item.find('.mg_author .summary-content a').map((i, e) => $(e).text()).get();
            const artists = $item.find('.mg_artists .summary-content a').map((i, e) => $(e).text()).get();
            const genres = $item.find('.mg_genres .summary-content a').map((i, e) => $(e).text()).get();
            const status = $item.find('.mg_status .summary-content').text().trim();
            const releaseYear = $item.find('.mg_release .summary-content a').text().trim();

            const latestChapter = $item.find('.latest-chap .chapter a').text().trim();
            const latestChapterUrl = $item.find('.latest-chap .chapter a').attr('href');
