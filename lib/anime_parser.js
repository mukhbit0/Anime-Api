        ...
        $('.uta').each((index, element) => {
            ...
            const chapters = $item.find('.luf ul.Manhwa li').map((j, e) => {
                ...
                return {
                    ...
                    c_url: $chapter.find('a').attr('href').replace('https://asuracomic.net/', ''),
                    c_date: $chapter.find('span').text().trim()
                };
            }).get();

            const modifiedUrl = url.replace('https://asuracomic.net/manga/', '');
...
const scrapeChapterData = async (id) => {
    try {
        ...
        $('#chapterlist .clstyle li').each((index, element) => {
            ...
            const chapterID = $chapter.find('a').attr('href').replace('https://asuracomic.net/', '');
...
const scrapeMangaDetail = async ({ id }) => {
    try {
        const mangaPage = await axios.get(`https://asuratoon.com/${id}`);
        ...
        const status = $('.imptdt:contains("Status")').find('i').text().trim();
        const type = $('.imptdt:contains("Type")').find('a').text().trim();
        const synopsis = $('.entry-content .messageContent__21e69').text().trim();
        const released = $('div.flex-wrap:contains("Released")').next().text().trim();
        const genres = $('.mgen a').map((index, element) => $(element).text().trim()).get();
        const artist = $('div.flex-wrap:contains("Artist")').next().text().trim();
        const author = $('div.flex-wrap:contains("Author")').next().text().trim();
