...
export const scrapeReaperChapterDataFromUrl = async (chapterUrl, mangaId) => {
    try {
        ...
        // Fetch chapter ID
        const chapterIdMatch = chapterUrl ? chapterUrl.match(/https?:\/\/[^/]+\/manga\/([^/]+)\/chapter-(.+)\/?$/) : null;
        const chapterId = chapterIdMatch ? `${chapterIdMatch[1]}-chapter-${chapterIdMatch[2]}` : null;
        chapterData.chapterId = chapterId;
